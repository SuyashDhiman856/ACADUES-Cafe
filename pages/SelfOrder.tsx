...
import { Search, Plus, Minus, Hash, Banknote, Smartphone, CheckCircle2, RefreshCw, Phone, LogOut, Coffee, ArrowRight, ArrowLeft, X, Layers, Clock, MessageCircle, Sparkles, Megaphone, Gift, Star } from 'lucide-react';
import { OrderStatus, PaymentMethod, OrderType, Order, ItemStatus, MenuItem, MenuVariant, Offer, CreateOrderDto } from '../types';
...
const SelfOrder: React.FC<SelfOrderProps> = ({ tenantId, initialOrder, initialStep = 1, offers = [], onCancel, onSave }) => {
  ...
  const addToCart = (item: MenuItem, variant?: MenuVariant) => {
    ...
    const finalId = variant ? `${item.id}-${variant.id}` : item.id;
  ...
  const handleProcessOrder = async (mode: 'KOT' | 'SETTLE') => {
    ...
    try {
      ...
      if (mode === 'SETTLE' && paymentMethod === PaymentMethod.UPI) {
        const upiLink = generateUPILink(displayOrder, settings);
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-2 flex flex-col flex-1">
                        ...
                        {item.description && <p className="text-[8px] text-gray-400 line-clamp-1 mb-1">{item.description}</p>}
