...
const NewOrder: React.FC<NewOrderProps> = ({ tenantId, initialOrder, initialStep = 1, menuItems, offers, settings, onCancel, onSave }) => {
  ...
  const [gstPercentage] = useState(initialOrder?.gstPercentage || settings.gstPercentage);
  ...
  const [orderId] = useState(() => {
    ...
    return `${settings.orderIdPrefix}-${ts}-${rand}`;
  });
  ...
  const addToCart = (item: MenuItem, variant?: MenuVariant) => {
    ...
    setCart(prev => {
      ...
      // We check if the ID starts with baseId
      const existingIndex = prev.findIndex(i => i.id.startsWith(baseId) && (i.status === ItemStatus.PENDING || i.status === ItemStatus.PREPARING));
      ...
      // Otherwise add a new line with a unique ID
  ...
  const removeFromCart = (item: MenuItem, variant?: MenuVariant) => {
    ...
    setCart(prev => {
      ...
      if (existingIndex > -1) {
        const actualIndex = prev.length - 1 - existingIndex;
  ...
  const updateQuantity = (itemId: string, delta: number) => {
    setCart(prev => {
      ...
      // If we are decreasing and quantity is 1, remove it
      ...
      // If we are adding to a READY/DELIVERED item, maybe we should create a new line instead?
      // But this function is called from the cart UI, where the user explicitly targets a line.
      // If they target a READY line and hit "+", they probably want another one of that, 
      // but it should be PENDING.
      
      if (delta > 0 && (item.status === ItemStatus.READY || item.status === ItemStatus.DELIVERED)) {
        // Instead of updating this line, we find if there's a PENDING line for this item
        const baseId = itemId.split('-').slice(0, -1).join('-') || itemId.split('-')[0];
        const pendingIndex = prev.findIndex(i => i.id.startsWith(baseId) && i.status === ItemStatus.PENDING);
        
        if (pendingIndex > -1) {
          updated[pendingIndex] = { ...updated[pendingIndex], quantity: updated[pendingIndex].quantity + 1 };
          return updated;
        } else {
          // Create new pending line
          const uniqueId = `${baseId}-${Math.random().toString(36).substring(2, 7)}`;
          return [...prev, { ...item, id: uniqueId, quantity: 1, status: ItemStatus.PENDING }];
        }
      }
      ...
      updated[existingIndex] = { 
        ...item, 
        quantity: newQty, 
        status: delta > 0 ? ItemStatus.PENDING : item.status 
      };
      return updated;
    });
  };
  ...
  const selectedOffer = useMemo(() => {
    ...
    if (offer && offer.minPurchase && subtotalAmount < offer.minPurchase) {
      return null; // Offer not eligible
    }
    return offer;
  }, [appliedOfferId, offers, subtotalAmount]);

  // Auto-deselect offer if no longer eligible
  React.useEffect(() => {
    if (appliedOfferId) {
      const offer = offers.find(o => o.id === appliedOfferId);
      if (offer && offer.minPurchase && subtotalAmount < offer.minPurchase) {
  ...
  const handleGetCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          // Optional: Try to get address using Nominatim (OpenStreetMap)
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 pb-32 no-scrollbar touch-pan-y" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                {filteredItems.map((item, index) => {
                  const itemsInCart = cart.filter(i => i.id.startsWith(item.id));
            ...
            <div className="max-w-xl mx-auto px-6 py-10 space-y-12 animate-in slide-in-from-bottom-6">
                      ...
                      return (
                        <button 
                          ...
                          className={`min-w-[200px] p-5 rounded-[32px] text-left border-2 transition-all flex flex-col gap-3 relative overflow-hidden group ${
                            ...
                            : isEligible 
                              ? 'border-[#F1E7E1] bg-white' 
                              : 'border-gray-100 bg-gray-50 opacity-60 grayscale cursor-not-allowed'
                          }`}
                        >
                          <div className={`p-2 rounded-xl w-fit ${appliedOfferId === offer.id ? 'bg-[#D17842] text-white' : isEligible ? 'bg-[#F9F5F2] text-[#D17842]' : 'bg-gray-200 text-gray-400'}`}>
                          ...
                          <div>
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`text-[8px] font-black px-1 rounded ${isEligible ? 'text-orange-600 bg-orange-100' : 'text-gray-400 bg-gray-200'}`}>
                                {offer.discountType === 'percentage' ? `${offer.discountValue}%` : `₹${offer.discountValue}`}
                              </span>
                            </div>
                            <p className="text-[8px] font-bold text-gray-400 mt-0.5 line-clamp-1">{offer.description}</p>
                            {!isEligible && (
                              <p className="text-[7px] font-black text-red-500 uppercase tracking-widest mt-1">Min. ₹{offer.minPurchase} required</p>
                            )}
                          </div>
                          {appliedOfferId === offer.id && (
              ...
              <div className="pt-10 space-y-4">
                ...
                <button 
                  ...
                  onClick={() => handleProcessOrder('KOT')} 
                  className="w-full flex items-center justify-center gap-3 py-7 rounded-[40px] text-white font-black uppercase tracking-[0.2em] transition-all text-xs bg-[#1C1C1E] shadow-2xl shadow-black/10 active:scale-95 disabled:bg-gray-200"
                >
                  {isSubmitting && submitMode === 'KOT' ? <RefreshCw className="animate-spin" size={24} /> : <Clock size={20} strokeWidth={3} />} 
                  Generate KOT
                </button>
                <button 
                  disabled={isSubmitting || cart.length === 0} 
